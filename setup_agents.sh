...
if [ ! -f ".env" ]; then
    ...
    echo "   - OPENAI_API_KEY"
    echo "   - DEEPSEEK_API_KEY (optional — strong reasoning)"
    echo "   - GOOGLE_AI_API_KEY (optional — fast summaries)"
...
echo "Testing Social Sentiment Agent..."
curl -s http://localhost:8005/ | python3 -m json.tool || echo "⚠️  Social Sentiment Agent not responding"
echo ""

# Test on-chain agent
echo "Testing On-Chain Agent..."
curl -s http://localhost:8006/ | python3 -m json.tool || echo "⚠️  On-Chain Agent not responding"
echo ""

# Test macro economics agent
echo "Testing Macro Economics Agent..."
curl -s http://localhost:8007/ | python3 -m json.tool || echo "⚠️  Macro Economics Agent not responding"
echo ""

# Test portfolio optimizer agent
echo "Testing Portfolio Optimizer Agent..."
curl -s http://localhost:8008/ | python3 -m json.tool || echo "⚠️  Portfolio Optimizer Agent not responding"
echo ""
...
echo "  • Social Sentiment:     http://localhost:8005"
echo "  • On-Chain Agent:       http://localhost:8006"
echo "  • Macro Economics:      http://localhost:8007"
echo "  • Portfolio Optimizer:  http://localhost:8008"
echo ""
echo "Documentation:"
