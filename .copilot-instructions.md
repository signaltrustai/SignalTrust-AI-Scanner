...
# Store keys with encryption
manager = KeyManager()
manager.set_key('OPENAI_API_KEY', 'sk-proj-...', save=True)

# Retrieve keys (auto-fallback to env vars)
...
/config/           # Configuration files
  /api_keys/      # Secure encrypted key storage
...
# API Key Management
python3 config/api_keys/key_manager.py    # View stored keys
python3 config/api_keys/key_validator.py  # Validate keys
```

## ☁️ Google Colab Integration
Launch SignalTrust AI in the cloud without installation:
...
See `SignalTrust_AI_Scanner.ipynb` and `COPILOT_COLAB_LINK_ENHANCED.md` for details.
...
### "Permission denied"
→ Run `chmod +x start.sh setup_agents.sh`
